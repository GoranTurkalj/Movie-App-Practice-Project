  ...
  <div id="user-rating-container">
    <ul ref="rating-list">
      <li
        ...
        :key="n"
        :data-starid="11 - n"
        @click="determineUserRating"
        @mouseenter="ratingOn = false"
        @mouseleave="ratingOn = true"
        :class="{goldenStar: 11-n <= getUserRating && ratingOn}"
      >
        <svg
          class="rating-star"
          height="511pt"
...
export default {
  ...
  computed: {
    ...mapGetters(["getUserRating"]),
  },

  methods: {
    ...mapMutations(["setUserRating"]),
    determineUserRating(event) {
      const rating = event.currentTarget.dataset.starid;
      console.log(rating);
      //Mutates user rating
